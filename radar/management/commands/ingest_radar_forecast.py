...
class Command(BaseCommand):
    help = "Ingest KNMI radar_forecast 2.0 HDF5 nowcast files."
    ...
    def handle(self, *args, **options):
        ...
        data_dir = Path(settings.KNMI_RADAR_FORECAST_DATA_DIR)
        client = KnmiOpenDataClient(
            api_key=api_key,
            dataset_name=settings.KNMI_RADAR_FORECAST_DATASET,
            dataset_version=settings.KNMI_RADAR_FORECAST_VERSION,
        )
    ...
    def _files_since(self, client: KnmiOpenDataClient, since: str, limit: int | None):
        ...
        if since_dt.tzinfo is None:
            raise CommandError("--since must include a timezone offset, e.g. 2026-08-30T00:00:00Z")
    ...
    def _ingest_file(self, client, data_dir: Path, file_info, force: bool) -> str:
        ...
        if existing and existing.status == RadarForecast.Status.PARSED and not force:
        ...
        try:
            ...
            metadata = parse_radar_forecast_hdf5(destination)
            issued_at = parse_filename_issued_at(file_info.filename)

            with transaction.atomic():
                forecast, _created = RadarForecast.objects.update_or_create(
                ...
                RadarForecastStep.objects.bulk_create(
                    [
                        RadarForecastStep(
                            forecast=forecast,
            ...
            RadarForecast.objects.update_or_create(
                filename=file_info.filename,
                defaults={
                    "issued_at": parse_filename_issued_at(file_info.filename),
                    "knmi_created": file_info.created,
                    ...
                    "size": file_info.size,
                    "file_path": str(destination),
                    "status": RadarForecast.Status.FAILED,
                    "error": str(exc),
                },
            )
            self.stderr.write(
